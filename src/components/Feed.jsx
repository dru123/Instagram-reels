...
function Feed() {
    ...
    const [isCommentActive, setCommentActive] = useState([]);
    ...
    const useStyles = makeStyles((theme) => ({
        ...
        selected:{
          ...
          userName:{
        marginLeft:'42rem',marginTop:"-12.5rem" ,position:"absolute",
  ...
  try{
    ...
    const handleLiked=async (puid)=>{
        ...
        if(isLiked==false){
            database.posts.doc(puid).update({
                "likes":[...likes,currUser.uid]
            })
          }  else{
              let likes=post.likes.filter(lkuid=>{
                  return lkuid!=currUser.uid;
              })
       database.posts.doc(puid).update({
           "likes":likes
       })
    ...
    const handlInputFile=(e)=>{
      ...
      const f2=()=>{
          alert("there was an error in uploadig the file");
          return;
      }
      //success
      ...
      const f3=()=>{
          uploadTask.snapshot.ref.getDownloadURL().then(async url=>{
              //2
              //post collection->post doc(firestore)
             ...
             //3.user postid should be update after every post ->with new post ids

             await database.users.doc(currUser.uid).update({
             ...
             uploadTask.on('state_chnaged',f1,f2,f3);
    }
   useEffect( async () => {
   console.log("hi");
...
<AppBar position="static" className={classes.app} >
   ...
   <IconButton>  <Avatar alt="Remy Sharp "  className={classes.avatar} src={user.profileUrl} onClick={<Profile></Profile>}></Avatar></IconButton> 
      </Link>
      <Button variant="contained" color="primary" style={{marginLeft:"5%"}} onClick={handleLogout} disabled={loader}>
  Logout
</Button>
    </Toolbar>
...
<input
        ...
        id="contained-button-file"
          multiple type="file"
          onChange={(e)=>{handlInputFile(e)}}
      />
      <label htmlFor="contained-button-file">
        <Button variant="outlined" color="secondary"  component="span" style={{width:"15rem",marginLeft:"0.5em",marginTop:"0.8rem"}}  startIcon={<BackupIcon></BackupIcon>} >
          UPLOAD Video
        </Button>
      </label>
{/* <button onClick={handleClick} disabled={loader} color="primary">Logout</button> */}
</div>
...
<div  className="feed">
    {videos.map((videoObj,idx)=>{
             ...
             return <div className="video-container" style={{display:"flex", position:"relative","justifyContent":"center"}}>
                <Video  
                videoObj={videoObj}
             src={videoObj.videoUrl}
             id={videoObj.puid}
             userName={videoObj.userName}
             userProfile={videoObj.userProfileUrl}
...
function Video(props){
    ...
    return (
      ...
      <Avatar alt="Remy Sharp " className={classes.videoImage} src={props.userProfile}
                    ...
                    <FavouriteIcon className={[classes.icon,classes.heart,props.isLiked==false?classes.notSelected:classes.selected]}
                    onClick={()=>{props.handleLiked(props.puid)}}></FavouriteIcon>
