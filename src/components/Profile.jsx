...
console.log(dataObject.data());
        ...
        let postid=user?.postIds;
        useEffect(async()=>{
           
           
            let dataObject=await database.users.doc(currUser.uid).get();
            let users=dataObject.data();
            let allPostData=users?.postIds;
            console.log(allPostData);
            let allPost=[];
            for(let i=0;i<allPostData.length;i++){
                console.log(allPostData[i]);
                let docRef=await database.posts.doc(allPostData[i]).get();
...
<div style={{borderBottom:"4px solid lightgray",height:"24vh"}}>
...
<div style={{display:"flex",justifyContent:"space-evenly"}} >
{
    videos.map((videosObj)=>{
       ...
       style= {{
        ...
        // display:"flex",padding:"6%",marginLeft:"8%"
        
        // justifyContent:"center",
        // alignItems:"center"
        }}
        controls id={videosObj.puid}
        >
                <source src={
                videosObj=={}?"":videosObj.url
                }type="video/mp4"
                ></source>
                </video>
