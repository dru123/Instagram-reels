// import React ,{useState,useContext} from 'react'
// import { AuthContext } from '../context/AuthContext';
// import { database,storage,firestore } from '../firebase';


// function Signup(props) {
//     const[email,setEmail]= useState("");
//     const[username,setUserName]=useState("");
//     const[password,setPassword]=useState("");
//     const[error,setError]=useState(false);
//     const[loader,setLoader]=useState(false);
//     const[file,setFile]=useState(null);
//     let {signup}= useContext(AuthContext);
//     function handleSubmit(e){
//     let file=e?.target?.files[0];
//      if(file!=null){
//          setFile(e.target.files[0]);
//      }

//     }
//      async function handleSignup(e){
//          e.preventDefault();
        
//          try{
//              setLoader(true);
//              let res=await signup(email,password);
//              let uid= res.uid;
//              console.log(uid);
             
//              console.log(res,storage);
//              //putting file on storage
//              //lisener provide krtaa gh file heavy hoti h uski progress batane kliye
//              const uploadTaskListener= storage.ref(`/users/${uid}/profileImage`).put(file);
//               //fn1->progress
//               //f2->error
//               //f3->success
              
//               uploadTaskListener.on('state_chnaged',fn1,fn2,fn3);
//               function fn1(snapshot){
//                 var progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
//                 console.log(progress);

//               }
//               function fn2(error){
//                      setError(error);
//                      setLoader(false);
//               }
//            async     function fn3(){
//                    let downloadurl=await
//                    uploadTaskListener.snapshot.ref.getDownloadURL();
//                    //database k ander doc bna diye userid  k thorugh 
                
//                    database.users.doc(uid).set({
//                     username,
//                         email:email,
//                         userId:uid,
//                         createdAt:database.getUserTimeStamp(),//time jb bna th
//                         profileUrl:downloadurl
//                    })
                  
//                    setLoader(false);
//                    props.history.push("/");
//               }
//          }
//          catch(err){
//              setError("");
//              setLoader(false);
//          }

//     }

//     return (
//       <div>
//             <form onSubmit={handleSignup}>
//                 <div>
//                       <label htmlFor="">Username</label>
//                       <input type="text"  value={username} 
//                       onChange={(e)=>{setUserName(e.target.value)}}></input>
//                   </div>
   
//               <div>
//                 <label htmlFor="">Email</label>
//                 <input type="email"  value={email} 
//                 onChange={(e)=>{setEmail(e.target.value)}}></input>

//               </div>
//               <div>
//                 <label htmlFor="">password</label>
//                 <input type="password"  value={password} 
//                 onChange={(e)=>{setPassword(e.target.value)}}></input>

//               </div>
//               <div>
//                 <label htmlFor="">Profile Image</label>
//                 <input accept='image/*'  type="file"
//                 onChange={(e)=>{handleSubmit(e)}}></input>

//               </div>
//               <button type="submit" disabled={loader}>Submit</button>
//         </form>
//       </div>
//     )
// }

// export default Signup
// import React, { useEffect, useState, useContext } from 'react'
// import { AuthContext } from '../context/AuthContext';
// import { storage, firestore, database } from "../firebase";

// function Signup(props) {
//     const [email, setEmail] = useState("");
//     const [username, setUserName] = useState("");
//     const [password, setPassword] = useState("");
//     const [error, setError] = useState(false);
//     const [loader, setLoader] = useState(false);
//     const [file, setFile] = useState(null);
//     let { signup } = useContext(AuthContext);

//     function handlFileSubmit(e) {
//         let file = e?.target?.files[0];
//         if (file != null) {
//             console.log(e.target.files[0])
//             setFile(e.target.files[0]);
//         }
//     }
//     async function handleSignup(e) {
//         e.preventDefault(); 
//         try {

//             setLoader(true);
//             // 1
//             let res = await signup(email, password);
//             let uid = res.user.uid;
//             const uploadTaskListener = storage.ref(`/users/${uid}/profileImage`).put(file);
//             // fn1 -> progress
//             // fn2 -> error 
//             // fn3-> success
//             uploadTaskListener.on('state_changed', fn1, fn2, fn3);
//             function fn1(snapshot) {
//                 var progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
//                 console.log(progress);
//             }
//             function fn2(error) {
//                 setError(error);
//                 setLoader(false);
//             }
//             async function fn3() {
//                 // link get 
//                 let downloadurl = await
//                     uploadTaskListener.snapshot.ref.getDownloadURL();
//                 database.users.doc(uid).set({
//                     email: email,
//                     userId: uid,
//                     username,
//                     createdAt: database.getUserTimeStamp(),
//                     profileUrl: downloadurl
//                 })
//                 setLoader(false);
//                 props.history.push("/")
//             }


//         } catch (err) {
//             setError("");
//             setLoader(false);
//         }
//     }
//     return (
//         <div>
//             <form onSubmit={handleSignup}>
//                 <div>
//                     <label htmlFor="">UserName</label>
//                     <input type="text" value={username}
//                         onChange={(e) => { setUserName(e.target.value) }} />
//                 </div>
//                 <div>
//                  <label htmlFor="">Email</label>
//                   <input type="email" value={email}
//                         onChange={(e) => { setEmail(e.target.value) }} />
//                 </div>
//                 <div>
//                     <label htmlFor="">Password</label>
//                     <input type="password"
//                         value={password}
//                         onChange={(e) => { setPassword(e.target.value) }}></input>
//                 </div>
//                 <div>
//                     <label htmlFor="">Profile Image</label>
//                     <input type="file" accept="image/*"

//                         onChange={(e) => { handlFileSubmit(e) }}></input>
//                 </div>
//                 <button type="submit" disabled={loader}>Login</button>
//             </form>
//         </div>
//     )
// }

// export default Signup

import React, { useEffect, useState, useContext } from 'react'
import { AuthContext } from '../context/AuthContext';
 import { storage, firestore, database } from "../firebase";
 import {makeStyles,Grid,Container,CardMedia,Card,
    CardActions,Button,Typography,TextField,CardContent} from '@material-ui/core'
    import BackupIcon from '@material-ui/icons/Backup';

import IconButton from '@material-ui/core/IconButton';

   import { LinkButton } from './Login';
function Signup(props) {
    const [email, setEmail] = useState("");
    const [username, setUserName] = useState("");
    const [password, setPassword] = useState("");
    const [error, setError] = useState(false);
    const [loader, setLoader] = useState(false);
    const [file, setFile] = useState(null);
    let { signup } = useContext(AuthContext);
        function handlFileSubmit(e) {
         console.log("hi")
        let file = e?.target?.files[0];
        if (file != null) {
            console.log(e.target.files[0])
            setFile(e.target.files[0]);
        }
    }
    async function handleSignup(e) {
        console.log("hi")
        e.preventDefault(); 
        console.log(email,password);
        try {
                console.log("hi231")
            setLoader(true);
            // 1
            let res = await signup(email, password);
            let uid = res.user.uid;
            console.log("signin",uid);
            const uploadTaskListener = storage.ref(`/users/${uid}/profileImage`).put(file);
            // fn1 -> progress
            // fn2 -> error 
            // fn3-> success
            uploadTaskListener.on('state_changed', fn1, fn2, fn3);
            function fn1(snapshot) {
                var progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
                console.log(progress);
            }
            function fn2(error) {
                setError(error);
                setLoader(false);
            }
            async function fn3() {
                // link get 
                let downloadurl = await
                    uploadTaskListener.snapshot.ref.getDownloadURL();
                database.users.doc(uid).set({
                    email: email,
                    userId: uid,
                    username,
                    createdAt: database.getUserTimeStamp(),
                    profileUrl: downloadurl,
                    postIds: []
                })
                setLoader(false);
                props.history.push("/")
            }


        } catch (err) {
            console.log(err);
            setError("");
            setLoader(false);
        }
    }
    let useStyles=makeStyles({
         centralDivs:{
             height:"100vh",
             
             width:"100vw",
             display:"flex",
             justifyContent:"center",
             alignItems:"center",
          
         },
         centreElemets:{
            display:"flex",
          flexDirection:"column",
         },
         image:{
             height:"6rem",
             backgroundSize:'contain'
         },
        
         alignCenter:{
             justifyContent:"center",
           
         },
         mb:{
             marginBottom:".8rem",
         },
         fullWidth:{
             width:"100%"
         },
        
        //  root: {
        //     '& > *': {
        //       margin: theme.spacing(1),
        //     },
        //   },
          input: {
            display: 'none',
          },
    })
    let classes=useStyles();
    return (
        <div  className={classes.centralDivs}>
          <Container >
              <Grid container   className={classes.alignCenter} >
                  <Grid item sm={5}  >
                  <Card  variant="outlined"   className={classes.mb}>
                  <CardMedia 
                   image="data:image/png;base64,"
                                title="Paella dish"
                                className={[classes.image]}
                               
                  />
               
                  <Typography style={{textAlign:"center",
                 color:"gray",
                 fontSize:"25px"}}>
                      Sign up to see photos and videos from your friends.
                  </Typography>
                  <CardContent className={classes.centreElemets}>
                  <TextField
                  value={email} id="outlined-basic"  type="email" label="Email" variant="outlined"   display="block"
                  className={classes.mb}
                  onChange={(e)=>{setEmail(e.target.value)}}/>
                  <TextField
                  value={password} id="outlined-basic" label="Password" variant="outlined" type="password" display="block"
                  className={classes.mb}
                  onChange={(e)=>{setPassword(e.target.value)}}/>
                  <TextField
                  value={username} id="outlined-basic" label="Full Name" variant="outlined" type="text" display="block"
                  className={classes.mb}
                  onChange={(e)=>{setUserName(e.target.value)}}/>
                </CardContent>
                <CardActions>

                
                    {/* <label >
                                  UPLOAD PROFILE IMAGE
                  <input type="file" accept="image/*"

            onChange={(e) => { handlFileSubmit(e) }}style={{display:"none"}}></input> */}
                             {/* </label> */}
                             <div >
      <input
        accept="image/*"
        className={classes.input}
        id="contained-button-file"
          multiple type="file"
          onChange={(e)=>{handlFileSubmit(e)}}
      />
      <label htmlFor="contained-button-file">
        <Button variant="outlined" color="secondary"  component="span" style={{width:"29.9rem",marginLeft:"0.3em"}}  startIcon={<BackupIcon></BackupIcon>} >
          UPLOAD PROFILE IMAGE
        </Button>
      </label>
      </div>
                </CardActions>
                <CardActions className={[classes.alignCenter,classes.mb]}>
                       <Button variant="contained" color="primary" onClick={(e)=>{handleSignup(e)}} style={{width:"25rem"}} > SIGN UP</Button>
                </CardActions>
                <Typography style={{textAlign:"center",
                 color:"#2E2E2E",
                 fontSize:"20px"}}>
                      By signing up,you agree to our Teams,Data Policy  and Cookies Policy.
                  </Typography>
                           </Card>
                           <Card>
                           <Typography style={{textAlign:"center"}}>
                    Have an account?
                    <LinkButton content="Log in"
                    route="/login"></LinkButton>
 
                </Typography>
                           </Card>
                           </Grid>
              </Grid>
          </Container>
        </div>
    )
}


export default Signup
